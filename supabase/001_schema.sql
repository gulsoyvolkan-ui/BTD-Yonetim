...
-- =============================================================================
-- DOĞRULAMA (ayrı query olarak da çalıştırılabilir)
-- =============================================================================
-- Beklenen: firmalar=3, banka=9, malzeme_gruplari>=8, belge_sayaclari=36
SELECT 'firmalar' AS tablo, COUNT(*)::TEXT AS adet FROM public.firmalar
UNION ALL SELECT 'firma_banka_hesaplari', COUNT(*)::TEXT FROM public.firma_banka_hesaplari
UNION ALL SELECT 'malzeme_gruplari', COUNT(*)::TEXT FROM public.malzeme_gruplari
UNION ALL SELECT 'belge_sayaclari', COUNT(*)::TEXT FROM public.belge_sayaclari
UNION ALL SELECT 'toplam_public_tablolar', COUNT(*)::TEXT
  FROM information_schema.tables
  WHERE table_schema = 'public' AND table_type = 'BASE TABLE';

SELECT kod, ad, unvan FROM public.firmalar ORDER BY kod;

-- =============================================================================
-- SONRAKİ ADIMLAR (kodlama vizyonu)
-- 1) GitHub BTD-Yonetim: bu SQL + sonraki supabase-js istemcisini commit/push
-- 2) Frontend: VITE/CDN ile @supabase/supabase-js · firmalar + login önce
-- 3) RLS: auth.uid() → kullanicilar → kullanici_firmalar → firma_id filtresi
-- 4) Modül taşıma sırası: firmalar → kullanıcı/RBAC → müşteri → teklif/sipariş
--    → iş emri → tedarik/RFQ → fason → kalite → sevkiyat → fatura → stok
-- =============================================================================
